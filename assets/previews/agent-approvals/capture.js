/* Two clips from one animation:
     node capture.js typing     command types out, then stops
     node capture.js output     command types out, then prints the four components  */
const puppeteer = require('puppeteer');
const { execSync } = require('child_process');
const path = require('path');
const fs = require('fs');

const MODE = process.argv[2] === 'output' ? 'output' : 'typing';
const DURATION = MODE === 'output' ? 5.5 : 3.2;
const FPS = 20;
const TOTAL_FRAMES = Math.round(DURATION * FPS);
const WIDTH = 360, HEIGHT = 225;

const HTML_PATH = path.resolve(__dirname, 'animation.html');
const FRAMES_DIR = path.resolve(__dirname, `frames-${MODE}`);
const OUT = path.resolve(__dirname, `agent-approvals-${MODE}.mp4`);
const OUT_GIF = path.resolve(__dirname, `agent-approvals-${MODE}.gif`);

if (fs.existsSync(FRAMES_DIR)) fs.rmSync(FRAMES_DIR, { recursive: true });
fs.mkdirSync(FRAMES_DIR);

(async () => {
  const browser = await puppeteer.launch({ headless: true, args: ['--no-sandbox'] });
  const page = await browser.newPage();
  await page.setViewport({ width: WIDTH, height: HEIGHT, deviceScaleFactor: 2 });
  await page.evaluateOnNewDocument((m) => { window.OUTPUT_MODE = (m === 'output'); }, MODE);
  await page.goto(`file://${HTML_PATH}`);
  await new Promise(r => setTimeout(r, 400));

  await page.evaluate(() => {
    document.getAnimations().forEach(a => { a.pause(); a.currentTime = 0; });
  });

  console.log(`${MODE}: capturing ${TOTAL_FRAMES} frames`);
  const frameMs = 1000 / FPS;
  for (let i = 0; i < TOTAL_FRAMES; i++) {
    await page.evaluate((ms) => {
      document.getAnimations().forEach(a => { a.currentTime = ms; });
    }, i * frameMs);
    await page.evaluate(() => new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r))));
    await page.screenshot({ path: path.join(FRAMES_DIR, `frame${String(i).padStart(4,'0')}.png`),
                            clip: { x:0, y:0, width:WIDTH, height:HEIGHT } });
  }
  await browser.close();

  execSync(`ffmpeg -y -framerate ${FPS} -i "${FRAMES_DIR}/frame%04d.png" \
    -vf "fps=${FPS},scale=${WIDTH}:-2:flags=lanczos,format=yuv420p" \
    -c:v libx264 -crf 18 -preset slow -movflags +faststart "${OUT}"`, { stdio: 'inherit' });

  execSync(`ffmpeg -y -framerate ${FPS} -i "${FRAMES_DIR}/frame%04d.png" \
    -vf "fps=${FPS},scale=${WIDTH}:-1:flags=lanczos,split[s0][s1];[s0]palettegen=max_colors=256:stats_mode=diff[p];[s1][p]paletteuse=dither=floyd_steinberg" \
    "${OUT_GIF}"`, { stdio: 'inherit' });

  fs.rmSync(FRAMES_DIR, { recursive: true });
  console.log('done:', OUT);
})();
