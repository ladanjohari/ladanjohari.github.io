...
const HTML_PATH = path.resolve(__dirname, 'animation-light.html');
const FRAMES_DIR = path.resolve(__dirname, 'frames-light');
const OUT_PREVIEW = path.resolve(__dirname, 'session-indicator-preview-light.mp4');
const OUT_THUMB   = path.resolve(__dirname, 'session-indicator-thumb-light.gif');
...
(async () => {
  ...
  // GIF for thumbnail (full res, CSS scales it)
  execSync(`ffmpeg -y -framerate ${FPS} -i "${FRAMES_DIR}/frame%04d.png" \
